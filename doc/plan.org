...
*** TODO StateMatrix:
***** TODO The objects should be created only once per paradigm
***** TODO On each trial, it once should 'reset-trasitions', and redefine them
      This way the list of states is unique (valid for all trials)
***** TODO Is a state exists, use it and update its transitions
...
***** TODO Save rawEvents as one array of times and one of integer transitions
*** TODO savedata.SaveData.to_file() should not have a parameter of type paramgui.Container
    because it is defined in another module and I should avoid dependencies.

* MINIPROJECT:
*** TODO Separate rawEvents into times and transitions.
