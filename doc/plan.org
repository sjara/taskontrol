...
***** TODO Start-stop-start on trial 0 causes error:
...
***** TODO Stop-start after a few trials causes trial outcome to be the same
...
********* call timeout() on dispatcher.resume()
...
*** TODO Move window when reaching the end.

* For main paradigm:
*** TODO Antibias: repeat-mistake and percent one side
*** TODO Randomize time before stimulus
*** TODO Randomize intensity of sound.
...
*** TODO Plot with average performance.

* To do soon:
*** TODO If the state matrix has not been created with all possible states at
...
*** TODO What happens if a new matrix is send while the machine is still running?
    Presumable it can create emptyt transitions or go crazy?
    This should only happen while on state 0, but what about other 'ready' states?
*** TODO Soundclient emulator (to test paradigms).
